...
description: "Quicksort 알고리즘이 시간복잡도 수학적으로 증명하기"
tags: [algorithm]
---

# 목표 정의
[이전 포스팅](quicksort-algorithm)에서 밝힌 바와 같이 Quicksort의 시간 복잡도는 최악의 경우 $$O(n^2)$$ 이며, pivot을 랜덤하게 선택하면 <b>평균적으로</b> $$O(n \log n)$$의 복잡도를 가진다. 알고리즘의 평균 복잡도가 $$O(n \log n)$$임을 수학적으로 증명한다.

# Worst case
Pivot을 선택할 때 항상 배열의 <b>최소값</b>을 선택한다고 하자. 이 경우 `Partition`과정 종료 후 좌측 부분배열은 공집합일 것이다. 이에 따라 한번의 iteration이 종료된 후 pivot을 제외한 $$n-1$$개의 원소들에 대해 알고리즘이 적용되게 된다.

이 경우 분할정복 알고리즘의 이점을 전혀 살리지 못한다. 각 iteration은 단순히 배열을 선형탐색한 후 오직 1개의 원소만을 제거할 뿐이다.

$$i+1$$번째 iteration에서는 $$n-i$$개의 원소를 탐색하게 되고 $$0 \geq i \geq n$$이므로 복잡도는 다음과 같다.
...
### 정의
1. $$\Omega$$: Quicksort의 표본공간. 즉 Quicksort에서 도출 가능한 모든 outcome들의 집합.
2. $$C(\sigma )$$: 주어진 Quicksort 시행 $$\sigma$$에서 발생한 element간의 비교 회수.

### Lemma
$$RT( \sigma )$$가 Quicksort $$ \sigma $$의 전체 Running time이라고 하자. 이 때 다음이 성립한다.

$$\exists c >0$$ $$s.t$$ $$\forall  \sigma \in \Omega$$ $$RT( \sigma ) \geq c C( \sigma)$$

직관적으로 이는 자명하다. 알고리즘에서 원소간 비교를 제외한 Swap은 상수번 발생하기 때문이다.
...
Quicksort는 모든 iteration에서 <b>pivot을 기준으로</b> 나머지 원소들을 비교한다. 두개의 element 중 1개가 pivot으로 선택된다면 선형탐색시 무조건 1번 비교되고 이후의 recursive call에서는 제외된다. 알고리즘 종결시까지 둘다 pivot으로 선택되지 못했다면, 애초에 두 원소는 서로 비교되지 않는다. 따라서 $$z_i$$와 $$z_j$$는 한번의 Quicksort 과정에서 무조건 <b>0회</b> 혹은 <b>1회</b> 비교된다.
...
$$C(\sigma)=\sum_{i=1}^{n-1}\sum_{j=i+1}^{n}X_{ij}(\sigma)$$ 이고,

기대값은 선형성을 가지므로

$$\therefore E[C(\sigma)]=\sum_{i=1}^{n-1}\sum_{j=i+1}^{n}E[X_{ij}(\sigma)]$$ 이다.
...
$$A$$의 어떤 원소도 pivot으로 선택되지 않는다면 둘은 같은 recursive call에 속한다. 따라서 $$A$$중 한 원소가 최초로 Pivot으로 선택될 경우를 생각하자.
...
# 정리
Random pivot을 활용한 Quicksort 알고리즘의 평균 복잡도는 Best case와 동일한 $$O(n \log n)$$ 이다. 배열의 원소들은 서로 최대 1회까지 비교된다는 사실을 활용하여 indicator variable을 선언하였고, <b>General decomposition principle</b>을 활용하여 평균을 계산할 수 있었다.
